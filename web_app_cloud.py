...
与本地版的区别：
  - 检索方式：纯 BM25 关键词检索（零外部依赖、零内存占用、中文友好）
  - Reranker：去掉（云端无需 1GB 本地模型）
  - BM25 优势：中文按字分词，公司名/术语精确匹配，无需下载任何模型
  - 其它功能不变：上传 PDF/TXT、聊天问答
...
# Cloud 版使用纯 BM25 关键词检索，不依赖任何外部嵌入模型/API
...
# Cloud 版：纯 BM25 关键词检索，无需嵌入模型，零内存占用
...
def build_system(_docs_hash: str, _top_k: int):
    """根据知识文本构建 BM25 关键词检索器（纯本地，零外部依赖）"""
    ...
    print(f"  ✓ 文档已切分: {len(documents)} 个片段")

    # 纯 BM25 关键词检索（中文按字分词，精确匹配效果好）
    class BM25OnlyRetriever:
        """纯 BM25 关键词检索器，无需嵌入模型"""
        def __init__(self, docs, k=5):
            self._bm25 = _BM25Retriever.from_documents(docs)
            self._bm25.k = k

        def invoke(self, query):
            return self._bm25.invoke(query)

    retriever = BM25OnlyRetriever(documents, k=_top_k)
    print(f"  ✓ BM25 检索器就绪: {len(documents)} 个文档片段")
...
def main():
    ...
    # ---- 主区域 ----
    st.title("🤖 RAG 知识库问答")
    st.caption("云端版 · BM25 关键词检索 · DeepSeek 生成 · 上传 PDF/TXT 即用")
    ...
    if st.session_state.knowledge_text and not st.session_state.system_ready:
        with st.spinner("🔄 正在初始化（向量化文档 + 构建检索器）..."):
            try:
                ...
                retriever, doc_count = build_system(docs_hash, top_k)
